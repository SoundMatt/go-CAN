...
|---|---|---|---|
| ASIL-B / SIL 2 | ≥ 80% per package (project target; enforced by review, not yet a hard CI gate — see note below, not yet met by every package) | Not separately measured | `go test -coverprofile` + `gofusa coverage` |
...
overall statement coverage is ~82%, with most packages in the high 80s to
high 90s (`mock` at 100%) and `cmd/cantool` the current low point at ~76%
(CLI argument-parsing and error-path branches are the main gap). The
`examples/quickstart/*` demo binaries have no tests and are intentionally
excluded from this target — they are not part of the library's safety-
relevant surface.
